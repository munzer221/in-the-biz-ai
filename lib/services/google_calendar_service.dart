...
class GoogleCalendarService {
  ...
  Future<bool> requestCalendarAccess() async {
    ...
    try {
      print('[v1.1.5] Starting requestCalendarAccess...');

      // Initialize GoogleSignIn singleton once
      if (!_initialized) {
        print('[v1.1.5] Initializing GoogleSignIn...');
        await GoogleSignIn.instance.initialize();
        _initialized = true;
      ...
      print('[v1.1.5] Attempting lightweight authentication...');
      ...
      if (lightweightUser != null) {
        ...
        print('[v1.1.5] Got user from lightweight auth: ${lightweightUser.id}');
      } else if (_currentUser != null) {
        print('[v1.1.5] Got user from event listener: ${_currentUser!.id}');
      } else {
        // No user available - on web, we need to request authorization directly
        // which will trigger the OAuth flow with account picker
        print(
            '[v1.1.5] No user found - requesting authorization directly (triggers OAuth)...');
        ...
        try {
          ...
          print(
              '[v1.1.5] Authorization completed, creating HTTP client directly...');
          ...
          print('[v1.1.5] Creating CalendarApi from direct authorization...');
          _calendarApi = calendar.CalendarApi(httpClient);
          ...
          print(
              '[v1.1.5] Calendar access granted successfully via direct auth!');
          return true;
        } catch (e) {
          print('[v1.1.5] Authorization failed: $e');
          return false;
        }
      }

      print('[v1.1.5] User found, requesting calendar scope authorization...');
      ...
      print('[v1.1.5] Authorization granted, creating HTTP client...');
      ...
      print('[v1.1.5] Creating CalendarApi...');
      _calendarApi = calendar.CalendarApi(httpClient);
      ...
      print('[v1.1.5] Calendar access granted successfully!');
      return true;
    } catch (e, stackTrace) {
